 ...
 * @method mixed hmset($key, $kvs) Set multiple hash fields to multiple values. <https://redis.io/commands/hmset>
...
class Redis extends Component
{
    ...
    const OPERATE_MASTER = 'master';

    const OPERATE_SLAVE = 'slave';
     ...
     * @var \Redis
     */
    protected $_master_redis_ins;

    /**
     * @var \Redis
     */
    protected $_slave_redis_ins;
    ...
    /**
     * @var array 主Redis服务器的相关信息
     * @see SentinelClient::masters()
     */
    protected $_master_redis_config;

    /**
     * @var array 从Redis服务器的相关信息
     * 是所有的从redis服务器中随机获取的一个
     * @see SentinelClient::slaves()
     */
    protected $_slave_redis_config;

    /**
     * @var bool 是否开启了redis的集群
     */
    protected $_cluster = false;

    /**
     * @var array 保存部分操作的默认参数,键值对
     * 比如 :默认超时时间 expire=>0
     */
    ...
    public function __construct($config = [])
    {
        ...
        if (isset($config['options'])) {
            if (is_array($config['options'])) {
                $this->options = ArrayHelper::merge($this->options, $config['options']);
            }
    ...
    {
        // 是否开启集群
        $this->_cluster = ArrayHelper::getValue($this->config, 'redisCluster');

        if ($this->_cluster) {
            $this->_sentinel = new Sentinel(['config' => $this->config]);
        } else {
            $config = ArrayHelper::getValue($this->config, 'master');
            $this->_slave_redis_config = $this->_master_redis_config = $config;
        }
    }
     ...
     */
    public function initConfig($operator = self::OPERATE_MASTER)
    {
        if ($operator === self::OPERATE_SLAVE) {
            if ($this->_slave_redis_config === null) {
                $this->_slave_redis_config = $this->_sentinel->getSlave();
            }
            if ($this->_slave_redis_ins === null) {
                $this->_slave_redis_ins = $this->connect($this->_slave_redis_config);
            }
            $this->_ins = $this->_slave_redis_ins;
        } else {
            if ($this->_master_redis_config === null) {
                $this->_master_redis_config = $this->_sentinel->getMaster();
            }
            if ($this->_master_redis_ins === null) {
                $this->_master_redis_ins = $this->connect($this->_master_redis_config);
            }
            $this->_ins = $this->_master_redis_ins;
        }
    }
    ...
    private function connect($config)
    {
        ...
        // 连接redis
        $redis_timeout = 3;
        $redis->connect($config['host'], $config['port'], $redis_timeout);
        $auth = ArrayHelper::getValue($this->config, 'auth');
        if (!empty($auth)) {
            $redis->auth($auth);
        }
        if (!$this->_cluster) {
            $db = ArrayHelper::getValue($this->config, 'master.database');
            if ($db > 0) {
                $redis->select($db);
            }
        }
    ...
    public function __call($command, $args)
    {
        ...
        if (in_array($command, $this->writeCommand)) {
            $this->initConfig(self::OPERATE_MASTER);
        } else if (in_array($command, $this->readCommand)) {
            $this->initConfig(self::OPERATE_SLAVE);
        } else if (in_array($command, $this->otherCommand)) {
            throw new InvalidArgumentException("Command must be executed on the specified connection");
        } else {
            throw new InvalidArgumentException("Command is not supported");
        }
        return call_user_func(array($this->_ins, $command), ...$args);
    }
